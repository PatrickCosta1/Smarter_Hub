...
Este manual foi escrito para colaboradores e utilizadores operacionais da Smarter Hub. O objetivo é que qualquer pessoa consiga usar a plataforma sem depender de alguém da equipa técnica para cada ação simples.
...
\hline
Colaboradores & Em PT-BR, o termo é o mesmo, mas algumas descrições podem usar \textit{pessoas} ou \textit{usuários internos}. \\
\hline
\end{longtable}

\chapter{Percurso do colaborador}
...
\section{O que o colaborador vê primeiro}
\begin{itemize}
...
\begin{enumerate}
  ...
  \item Procurar a formação pelo nome ou pelo colaborador.
...
Esta é uma das áreas mais usadas por qualquer colaborador. Serve para pedir férias, registar ausências e acompanhar o resultado do pedido.
...
Os dias automáticos representam dias dados pela empresa, por país e por ano. Normalmente não são criados pelo colaborador comum, mas aparecem no calendário para ajudar a leitura do período.
...
\subsection{Dias automáticos}
Este ecrã é informativo para o colaborador, mas é operacionalmente sensível para RH. O colaborador deve entender que um dia automático não é um pedido individual, é uma regra da empresa aplicada ao calendário.
...
\hline
A Minha Ficha & Ficha pessoal editável do colaborador. \\
\hline
...
\hline
A Minha Ficha & Colaborador autorizado & \texttt{/profile} & Etapas, checklist, guardar, submeter pedido & PT-PT usa linguagem de ficha; PT-BR entende-se como cadastro/perfil. \\
\hline
Plano de Carreira & Colaborador autorizado & \texttt{/plano-carreira} & O meu nível, roadmap, avaliação & Conteúdo de progressão deve ser lido com a terminologia interna do país. \\
\hline
...
\subsection{Exemplo de uso}
Um colaborador muda de morada. Abre A Minha Ficha, entra em dados de contacto, altera a morada habitual/fiscal e confirma a gravação.
...
\subsection{Exemplo de uso}
Um colaborador quer perceber o que significa o seu nível atual. Abre o separador correspondente e compara o roadmap com o processo de avaliação.
...
\subsection{Exemplo de uso}
O colaborador procura todas as formações ativas do último ano. Filtra por estado e lê a coluna de origem para entender quem criou o registo.
...
\subsection{Exemplo de uso}
O colaborador quer férias em agosto. Começa pelo resumo para ver saldo, depois consulta o calendário e, por fim, submete o pedido com observação curta.
...
\subsection{Exemplo de uso}
O colaborador procura o formulário de reporte de assédio, lê as instruções e submete uma descrição factual com contacto preferencial, se desejar.
...
\subsection{Exemplo de uso}
Uma notificação avisa que uma alteração de ficha foi rejeitada. O colaborador abre a notificação, lê o motivo e regressa à ficha para corrigir o campo indicado.
...
\begin{itemize}
  \item Um colaborador quer saber se o pedido de férias foi aprovado: consulta Notificações e depois Calendário.
  \item Um colaborador quer atualizar o telefone: abre A Minha Ficha, altera o campo e guarda.
  \item Um colaborador quer consultar conteúdos de ergonomia: entra em Saúde e bem-estar e abre o bloco correspondente.
  \item Um colaborador quer ver o seu percurso: entra em Plano de Carreira e lê o roadmap.
\end{itemize}
...
\chapter{Edição Brasil: operação local detalhada}
\section{Campos e documentos mais relevantes no Brasil}
No Brasil, os dados cadastrais e fiscais abaixo merecem validação prioritária:
\begin{itemize}
  \item CPF.
  \item PIS.
  \item CTPS, série e data de expedição.
  \item RG, órgão emissor e data de expedição.
  \item CNH, categoria e validade.
  \item Título de eleitor, zona e seção.
  \item Estado de trabalho (SP, RS, ou outro contexto BR configurado).
\end{itemize}

\begin{manualnote}[Boas práticas de cadastro no Brasil]
Valide primeiro identidade (CPF/RG), depois vínculo documental (CTPS/CNH) e por fim dados de contato e endereço. Essa ordem reduz retrabalho no fluxo de validação.
\end{manualnote}

\section{Férias e ausências no Brasil}
\subsection{Como montar o pedido}
\begin{enumerate}
  \item Abrir Férias / Ausências.
  \item Conferir saldo no Resumo.
  \item Verificar calendário com atenção para feriados e dias automáticos.
  \item Considerar âmbito geográfico, quando aplicável.
  \item Enviar o pedido com justificativa objetiva.
\end{enumerate}

\subsection{Pontos de atenção no contexto BR}
\begin{itemize}
  \item Alguns dias automáticos podem ser nacionais, outros podem ser específicos por estado.
  \item A linguagem de motivos de ausência pode variar conforme política local da empresa.
  \item O planejamento de equipe precisa considerar cobertura operacional por região.
\end{itemize}

\section{Saúde e bem-estar no Brasil}
\subsection{Blocos comuns entre países}
\begin{itemize}
  \item Formulário de assédio.
  \item Ergonomia.
  \item Suporte Básico de Vida.
...
\subsection{Blocos locais possíveis}
\begin{itemize}
  \item Diretrizes de atendimento local.
  \item Materiais de benefícios de saúde específicos do Brasil.
  \item Orientações regionais de suporte.
\end{itemize}

\chapter{Brasil: fluxos completos exemplificados}
\section{Fluxo A: atualizar documentos de identificação}
\begin{enumerate}
  \item Abrir A Minha Ficha.
  \item Ir para a seção fiscal/documental.
  \item Atualizar CPF, RG e CTPS, quando necessário.
  \item Confirmar datas e órgãos emissores.
  \item Salvar e acompanhar o status em notificações.
\end{enumerate}

\section{Fluxo B: férias com validação de equipe}
\begin{enumerate}
  \item Abrir Férias / Ausências.
  \item Conferir saldo no Resumo.
  \item Avaliar disponibilidade no calendário da equipe.
  \item Registrar o período e enviar.
  \item Acompanhar decisão nas notificações.
\end{enumerate}

\section{Fluxo C: uso do formulário de assédio}
\begin{enumerate}
  \item Abrir Saúde e bem-estar.
  \item Selecionar o bloco de formulário.
  \item Escrever descrição clara e objetiva.
  \item Informar contato preferencial, se desejar.
  \item Enviar e aguardar encaminhamento interno.
\end{enumerate}
