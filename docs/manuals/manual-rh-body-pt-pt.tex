...
A Smarter Hub reúne os principais processos de pessoas: membros, permissões, férias, ausências, formações, admissões, banco de horas, notificações, saúde e bem-estar e dashboards de acompanhamento.
...
\begin{itemize}
  \item Confirme sempre o país do membro antes de editar ou aprovar.
...
O Dashboard não serve para operações transacionais do dia a dia. É uma zona de leitura estratégica. Quando o problema for um membro concreto, o ponto certo costuma ser Membros, Aprovações, Férias / Ausências ou Admissões.

\chapter{Gestão de membros}
\section{Membros}
...
\begin{itemize}
  \item Pesquisar membros por nome, email, equipa ou estado.
  \item Abrir a ficha completa de um membro.
...
Em muitos casos o detalhe do membro agrupa-se em três blocos de trabalho:
\begin{itemize}
...
Use a área de permissões quando o membro precisar de acesso adicional, quando um acesso tiver de ser revogado ou quando a revisão de poder de ação fizer parte do processo de RH.
...
\begin{itemize}
  ...
  \item \textbf{Admissões}: validação de novos membros.
\end{itemize}
...
\begin{itemize}
  ...
  \item Confirmar sempre se o pedido corresponde ao membro certo.
\end{itemize}

\chapter{Admissões}
\section{Admissões}
O módulo de admissões acompanha o onboarding e a validação de novos membros.
...
O Banco de Horas serve para acompanhar créditos, débitos e saldos. É um módulo sensível porque qualquer movimento altera o histórico do membro.

\subsection{Separadores típicos}
\begin{itemize}
  \item \textbf{Meu Saldo}: leitura do membro.
...
\subsection{Resumo}
O resumo é o ecrã de leitura imediata. Mostra o essencial para perceber se o membro tem dias disponíveis, pedidos pendentes ou consumos já registados.
...
\begin{itemize}
  ...
  \item Ausências e férias devem ser avaliadas no contexto do tipo de pedido e do fluxo aplicável.
  \item Dias automáticos não devem ser tratados como pedidos normais de membro.
\end{itemize}
...
\begin{itemize}
  ...
  \item Ação de atribuir a um ou vários membros, quando permitido.
...
\section{Validação de dados de membro}
\begin{enumerate}
  \item Procurar o membro.
  \item Abrir a ficha.
...
\section{Detalhe do membro}
O detalhe do membro funciona como centro de comando. A partir daqui RH pode trabalhar com a ficha, com as permissões e com o estado da conta.
...
O Dashboard não substitui a ficha do membro. Ele ajuda a responder a perguntas mais amplas: onde estão os maiores grupos, que distribuição existe, que tendências aparecem e onde pode haver risco ou oportunidade.
...
\textbf{PT-PT} & \textbf{PT-BR / observação} \\
\hline
Membros & Membros / pessoas / usuários internos, dependendo do contexto. \\
\hline
Chefes de equipa & Líderes de equipe / gestores de equipe. \\
\hline
...
\hline
Membros & RH / gestão & \texttt{/membros} & Ficha, permissões, estado, importar, exportar & PT-PT usa muito ficha; PT-BR tende a usar cadastro/perfil na conversa. \\
\hline
...
Usar o Dashboard para confirmar um dado individual que deveria ser revisto na ficha do membro.

\section{Membros em detalhe}
...
\begin{itemize}
  ...
  \item Admissões: entrada de novos membros.
\end{itemize}
...
\subsection{Erro típico}
Lançar horas no membro errado ou sem descrever o motivo de forma verificável.
...
\begin{itemize}
  ...
  \item Procurar por membro, origem ou entidade.
...
\begin{enumerate}
  \item O membro submete uma alteração.
...
\chapter{RH Portugal: operação local aprofundada}
\section{Foco documental e fiscal em PT}
Para RH em Portugal, estes campos devem ser auditados com especial rigor:
\begin{itemize}
  \item NIF, NISS, Cartão de Cidadão e validade.
  \item Situação IRS, número de dependentes e declaração IRS.
  \item IBAN e comprovativo IBAN.
  \item Registo criminal (ficheiro anexo) para validação documental PT.
  \item Comprovativos de morada e documentação obrigatória.
\end{itemize}

\section{Matriz de decisão em aprovações (PT)}
\begin{longtable}{|p{0.24\textwidth}|p{0.24\textwidth}|p{0.2\textwidth}|p{0.24\textwidth}|}
\hline
\textbf{Tipo de pedido} & \textbf{Condição mínima} & \textbf{Ação} & \textbf{Justificação recomendada} \\
\hline
Alteração de ficha & Campos e comprovativos coerentes & Aprovar & Registo validado com documentação consistente \\
\hline
Alteração de ficha & Falta de comprovativo crítico & Rejeitar / pedir correção & Indicar campo e documento em falta \\
\hline
Férias & Sem conflito crítico de equipa & Aprovar & Planeamento compatível \\
\hline
Férias & Conflito operacional elevado & Rejeitar / negociar período & Necessidade de cobertura operacional \\
\hline
Admissão & Dados e contrato fechados & Concluir & Processo completo \\
\hline
\end{longtable}

\section{Fluxos exclusivos com exemplos (PT)}
\subsection{Fluxo PT-1: validação de IRS}
\begin{enumerate}
  \item Abrir Membros.
  \item Entrar no detalhe da pessoa.
  \item Validar situação IRS e dependentes.
  \item Confirmar comprovativo associado.
  \item Gravar e registar decisão.
\end{enumerate}

\subsection{Fluxo PT-2: revisão de férias com dias automáticos}
\begin{enumerate}
  \item Abrir Férias / Ausências.
  \item Conferir calendário e dias automáticos ativos.
  \item Verificar impacto na equipa.
  \item Aprovar ou rejeitar com motivo claro.
\end{enumerate}

\section{Erros críticos a evitar em PT}
\begin{itemize}
  \item Aprovar alteração sem ler comprovativos.
  \item Tratar dados fiscais como se fossem opcionais.
  \item Ignorar contexto da equipa em períodos de férias sensíveis.
\end{itemize}
