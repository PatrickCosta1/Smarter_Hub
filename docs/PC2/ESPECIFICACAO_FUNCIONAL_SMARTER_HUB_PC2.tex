...
\begin{itemize}
  \item O banco de horas é aplicável a colaboradores com \texttt{workCountry = BR}.
  \item Unidade oficial do saldo: \textbf{horas} (não minutos), com precisão de 2 casas decimais.
  \item Estrutura de saldo por colaborador: \textit{saldo creditado}, \textit{saldo debitado}, \textit{total} e \textit{limite}.
  ...
  \item Campo obrigatório de localização BR na ficha: \texttt{brWorkState} com opções \texttt{SP} e \texttt{RS}.
  \item Política de fecho por estado: \texttt{RS} semestral (abril/outubro); \texttt{SP} quadrimestral (fevereiro/junho/outubro).
  \item Folga por compensação: quando ausência com motivo \textit{Justificada - Folga} é aprovada, o sistema debita automaticamente horas no banco.
  \item Conversão automática para débito de folga: 1 dia aprovado = 8 horas debitadas.
  \item Excedente é calculado por \( |total| - limite \) quando positivo.
  \item Excedente gera aviso para o colaborador e para a chefia responsável, com cópia para utilizadores de acesso total.
